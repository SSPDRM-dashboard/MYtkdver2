...
async function run() {
  ...
  const targets = ["B14", "2014", "B15", "2015", "B16", "2016"];

  console.log("Searching for B14, B15, B16 variations across all rows in spreadsheet:");
  rows.slice(1).forEach((r, idx) => {
    const b = r[boutIdx]?.trim().toUpperCase();
    if (targets.includes(b)) {
      console.log(`Row ${idx+2}: Event: "${r[eventIdx]}" | Ring: "${r[ringIdx]}" | Bout: "${r[boutIdx]}" | Category: "${r[4]}" | Blue: "${r[5]}" vs Red: "${r[7]}" | Winner: "${r[15]}"`);
    }
  });
