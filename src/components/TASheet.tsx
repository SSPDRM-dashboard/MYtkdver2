        ...
        complete: (result) => {
          ...
          for (let i = 1; i < rows.length; i++) {
            ...
            if (row.length >= 10 && row[2] && row[3]) { 
              ...
              const existingMatchIdx = parsedMatches.findIndex(m => 
                String(m.ringNo) === String(row[2]) && 
                normalizeBoutNumber(m.matchNo) === normalizeBoutNumber(matchNo)
              );
              if (existingMatchIdx !== -1) {
                parsedMatches[existingMatchIdx] = newMatchItem;
              } else {
                parsedMatches.push(newMatchItem);
              }
              ...
              if (winner && winner.trim() && winner !== '-') {
                ...
                const existingHistIdx = historyItems.findIndex(h => h.id === historyId);
                if (existingHistIdx !== -1) {
                  historyItems[existingHistIdx] = historyItem;
                } else {
